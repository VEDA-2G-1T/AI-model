...
- **안전장비 착용 감지**: 헬멧, 안전조끼, 보안경 등 필수 안전장비 착용 여부 실시간 모니터링
- **넘어짐 감지**: 작업자 넘어짐 상황 감지 및 알림
- **실시간 처리**: 라즈베리파이 환경에서 실시간 영상 처리 및 분석
- **데이터 로깅**: 탐지 결과를 데이터베이스에 저장하여 이력 관리

## 시스템 아키텍처

```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   카메라 입력    │───▶│   AI 모델 추론   │───▶│   결과 출력      │
│   (RTSP/카메라)  │    │   (YOLO/TFLite) │    │   (화면/DB)     │
└─────────────────┘    └─────────────────┘    └─────────────────┘
                              │
                              ▼
                       ┌─────────────────┐
...
- **추론 엔진**: ONNX Runtime, TFLite Interpreter
- **최적화**: INT8 양자화, XNNPACK 델리게이트

### 개발 환경
- **언어**: Python 3.9, C++17
- **빌드**: CMake, Make
- **라이브러리**: OpenCV, GStreamer, SQLite3

### 하드웨어
- **학습**: NVIDIA RTX 4090 (CUDA)
- **배포**: Raspberry Pi 4 (ARM64)
...
*   **7/22 (화)**: `raspi_tflite/main.cpp`에 TFLite 모델로 추론하는 기본 로직 구현.
*   **7/23 (수)**: ~Server와 통합 진행행
